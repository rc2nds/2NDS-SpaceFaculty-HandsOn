...
void sendAll() {
  ...
  LoRa.print(vbat);
  LoRa.println(" V");
  LoRa.print("[Team Name] STATUS = ");
  if (!obcTempWarning && !epsTempWarning && !vbatWarning) {
    LoRa.println("NOMINAL");
  } else {
    if (obcTempWarning) {
      LoRa.print("WARNING! TEMPOBC is too high: ");
      LoRa.print(obcTemp);
      LoRa.println(" deg!");
    }
    if (epsTempWarning) {
      LoRa.print("WARNING! TEMPEPS is too high: ");
      LoRa.print(epsTemp);
      LoRa.println(" deg!");
    }
    if (vbatWarning) {
      LoRa.print("WARNING! VBAT is too low: ");
      LoRa.print(vbat);
      LoRa.println(" V!");
    }
  }
